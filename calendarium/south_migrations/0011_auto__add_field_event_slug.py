...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.add_column('calendarium_event', 'slug',
                      self.gf('django.db.models.fields.SlugField')(default='default',  blank=True),
                      keep_default=False)
        # Auto-generating slugs
        for event in orm.Event.objects.all():
            event.slug = orig = slugify(event.title)
            for x in itertools.count(1):
                if not orm.Event.objects.filter(slug=event.slug).exists():
                    break
                event.slug = '%s-%d' % (orig, x)
            event.save()
        # Changing field 'Event.slug'
        db.alter_column('calendarium_event', 'slug', self.gf('django.db.models.fields.SlugField')(unique=True))

    def backwards(self, orm):
        # Deleting field 'Event.slug'
        db.delete_column('calendarium_event', 'slug')
    ...
    models = {
        ...
        'calendarium.event': {
            ...
            'slug': ('django.db.models.fields.SlugField', [], {'max_length': '256', 'unique': 'True', 'blank': 'True'})
        },
